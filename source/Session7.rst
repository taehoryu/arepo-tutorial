...
.. code-block:: console
   ...
   $ helm_table.dat  ic_binary.py  ic_binary.ipynb  module.py  param_config_sample  plot_binary.py  species55.txt
...
Like ``ic_MS.py``, ``ic_binary.py`` or ``ic_binary.ipynb`` is written such that it generates initial condition files and puts all the necessary files in a new directory authomatically. You just need to make sure that ``path_to_single_relaxed_star`` is properly set to one of the output files from the simulation for the single star relaxation.
